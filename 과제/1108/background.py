...
class VertScrollBackground:
    ...
    def draw(self):
        ...
        curr = int(-self.scroll) % page
        if curr > 0:
            sh = int(1 + self.image.h * curr / self.ch)
            sl = self.image.h - sh
            src = 0, 0, self.image.w, self.image.h
            dh = int(sh * self.cw / self.image.w)
            if self.imageName == 'clouds.png':
                dst = 0, curr - 600, self.cw, self.ch / 3
            else:
                dst = 0, curr - 600, self.cw, self.ch
            self.image.clip_draw_to_origin(*src, *dst)
        dst_height = round(self.image.w * self.ch / self.image.h)
        while curr + dst_height < self.cw:
        ...
        if curr < self.ch:
            ...
            dh = int(sh * self.cw / self.image.w)
            if self.imageName == 'clouds.png':
                dst = 0, curr + 200, self.cw, self.ch / 3
            else:
                dst = 0, curr + 200, self.cw, self.ch
            self.image.clip_draw_to_origin(*src, *dst)
